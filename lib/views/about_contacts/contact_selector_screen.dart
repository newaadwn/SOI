...
class _ContactSelectorScreenState extends State<ContactSelectorScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!_isInitialized) {
        _contactController.requestContactPermission();
        _contactController.loadAddedContacts();
        _isInitialized = true;
      }
    });
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed &&
        _contactController.permissionDenied) {
      _contactController.requestContactPermission();
    }
  ...
  void _onSearchChanged() {
    _contactController.searchContacts(_searchController.text);
  }
              ...
              _searchController.text.isNotEmpty
                  ? IconButton(
                    ...
                    onPressed: () {
                      ...
                      _contactController.searchContacts('');
                    },
                  )
                  : null,
  ...
  // 연락처 목록 위젯
  Widget _buildContactsList(ContactController controller) {
    // 로딩 중인 경우
    ...
    // 연락처가 없는 경우
    if (controller.deviceContacts.isEmpty) {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        itemCount: controller.filteredDeviceContacts.length,
        itemBuilder: (context, index) {
          Contact contact = controller.filteredDeviceContacts[index];
          String phoneNumber =
  ...
  Future<void> _addContact(Contact contact) async {
    ...
    // 연락처 추가 시도
    final bool success = await _contactController.addContactFromDevice(contact);
  ...
  void _showContactDetails(Contact contact) {
    final String phoneNumber =
        contact.phones.isNotEmpty ? contact.phones.first.number : '';
    final bool isAdded = _contactController.isContactAdded(phoneNumber);
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // 프로필 이미지 또는 이니셜
              _contactController.buildContactAvatar(contact, radius: 40),
                ...
                ...contact.phones.map((phone) {
                  return Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          _contactController.formatPhoneNumber(phone.number),
